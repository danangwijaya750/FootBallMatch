...
class NextMatch : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        recNext.layoutManager=linearLayoutManager
        getData()
        recNext.adapter=eventAdp
    }
    fun getData(){
        val gson= Gson()
        val data=gson.fromJson(sendRequest("https://www.thesportsdb.com/api/v1/json/1/eventsnextleague.php?id=4328"), EventResponse::class.java)
        showList(data.events)
        Logger.getLogger(this.javaClass.name).warning(data.events.toString())
    }
